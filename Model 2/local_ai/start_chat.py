...
def selected_run(root=ROOT):
    candidate = root / 'runs/real_v2'
    if any((candidate / name).is_file() for name in ('plan.json', 'running.lock', 'status.json')):
        return candidate
    return root / 'runs/real_v1'
...
def snapshot(run=RUN):
    ...
    if active:
        ...
        if phase in ('pretraining', 'sft'):
            ...
            total = metrics.get('steps', status.get('planned_steps',
                                plan.get('sft_steps', 0) if phase == 'sft' else 0))
            if total:
                message += f': {step:,} / {total:,} Schritte ({100*step/total:.1f}%)'
            if phase == 'pretraining':
                message += '. Danach folgt das Dialogtraining.'
            if plan.get('target_tokens'):
                processed = plan.get('parent', {}).get('total_tokens', 0) + metrics.get('total_tokens', 0)
                if phase == 'sft':
                    processed += latest_metrics(run / 'pretrain/metrics.jsonl').get('total_tokens', 0)
                message += f' Verarbeitete Tokens: {processed:,} / Ziel {plan["target_tokens"]:,}.'
        if (run / 'STOP').exists():
    ...
    if phase == 'failed':
        return 'blocked', ('Training abgebrochen: ' + str(status.get('error', 'Unbekannter Fehler'))
                           + f'\nDetails: {run / "training-error.log"}')
    if phase == 'paused':
...
def main():
    ...
    run = selected_run()
    if args.status:
        print(snapshot(run)[1])
        return 0
    try:
        checkpoint = wait_for_chat(run)
