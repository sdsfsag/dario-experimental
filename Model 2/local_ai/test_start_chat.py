...
def test_run_selection_switches_when_continuation_actually_starts(tmp_path):
    assert start_chat.selected_run(tmp_path) == tmp_path/'runs/real_v1'
    new = tmp_path/'runs/real_v2'
    new.mkdir(parents=True)
    assert start_chat.selected_run(tmp_path) == tmp_path/'runs/real_v1'
    (new/'running.lock').write_text('{"pid": 12345}')
    assert start_chat.selected_run(tmp_path) == new


def test_continuation_counter_includes_parent_and_both_new_phases(tmp_path, monkeypatch):
    monkeypatch.setattr(start_chat, 'training_alive', lambda pid: bool(pid))
    state(tmp_path, 'sft')
    (tmp_path/'plan.json').write_text(json.dumps({'target_tokens': 1000, 'parent': {'total_tokens': 100}}))
    for stage, count in (('pretrain', 200), ('sft', 50)):
        (tmp_path/stage).mkdir()
        (tmp_path/stage/'metrics.jsonl').write_text(json.dumps({'step': 1, 'steps': 2, 'total_tokens': count}))
    assert '350 / Ziel 1,000' in start_chat.snapshot(tmp_path)[1]
